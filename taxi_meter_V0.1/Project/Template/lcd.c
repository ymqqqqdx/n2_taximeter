...
extern unsigned char tab_date[8]="2011-1-1";
extern unsigned char tab_wait_time[5]="00:00";
extern unsigned char tab_fee_calc[5]="355.5";
extern unsigned char tab_distance_calc[5]="200.2";
extern unsigned char tab_speed_calc[6]="60km/h";
extern unsigned char tab_speedsize[1]="2";
...
* 调用方法：ili9320_PutChar(10,10,'a',0x0000,0xffff);
****************************************************************************/
void ili9320_PutChars_S3264(u16 x,u16 y,uc8 *z,u8 len,u16 charColor,u16 bkColor)
{
  u16 i=0;
  u16 j=0;
  u8 kk = 0;
	...
	z=z++;
  		for (i=0;i<64;i++)
		  {
			...
			for(kk = 0; kk<4;kk++)
			{
				...
				else   tmp_char=ASCII_Data[((c-'0')*256)+kk+4*i];
...
/*void add(unsigned int x1,unsigned int y1,unsigned int x2,unsigned int y2,const unsigned char a[],unsigned char b[])
{
	  int i = 0;
	  int j = 0;
	  u32 count = 0;
...
/******************************************************************************/	 
/*float cal_money(float distance,long wait_time, u8 day)
{
    float money;
  ...
  /*******************************************************************************
* Function Name  : cal_money
* Description    :calculate money the passageer should pay 
* Input          : None
* Output         : None
* Return         : None
*******************************************************************************/
 ...
 */
//	ili9320_PutChars(100,100,tab_wel,8,0xF800,Green); //字符串
//	ili9320_PutChars(100,116,tab_date,8,0xF800,Green);
//	ili9320_PutChars(100,132,tab_time,8,0xF800,Green);
//	ili9320_PutChar(0,32,'b',0x0000,0xFFE0); //单个字符
//	ili9320_PutChar(10,32,'c',0x3000,0xF800); //单个字符
//	ili9320_PutCharz(0,70,18,0x4000,0x4fff); //显示18个汉字
//	LCD_Clear(Green);
/*	Delay(0xaffff);	
	Delay(0xaffff);
	Delay(0xaffff);
*//*
	ili9320_PutChars(0,0,tab_date,8,0xF800,Green);
	ili9320_PutChars(0,16,tab_time,8,0xF800,Green);
 
	ili9320_PutChars(167,0,tab_wait,19,0xF800,Green); //字符串
	ili9320_PutChars(257,16,tab_wait_time,8,0xF800,Green); //字符串

	ili9320_PutChars(64,64,tab_fee,13,0xF800,Green); //字符串
	ili9320_PutChars(168,80,tab_fee_calc,8,0xF800,Green); //字符串

	ili9320_PutChars(64,128,tab_distance,17,0xF800,Green); //字符串
	ili9320_PutChars(180,144,tab_distance_calc,4,0xF800,Green); //字符串

	ili9320_PutChars(223,160,tab_speed,12,0xF800,Green); //字符串
	ili9320_PutChars(240,176,tab_speed_calc,6,0xF800,Green); //字符串

	ili9320_PutChars(0,223,tab_fee_ratio,20,0xF800,Green); //字符串

*/	
	//LCD_DrawRectFill(0,0,100,100,Red);
//	 LCD_SetDisplayWindow(10,10,20,20) ;
//	LCD_DrawLine(10,10,240,0x01);
//	LCD_DrawLine(10,10,240,0x00);
	address_set(0,0,239,319);
		for(i=0;i<76800;i++)
		{
			dd=(0x00|gImage_x[i*2+1])<<8;
	...
	sprintf(tab_distance_calc,"%5.1f",distance);
	ili9320_PutChars_S3264(60+32*2,0,tab_distance_calc,5,Red,Green);	 //licheng



	sprintf(tab_fee_calc,"%5.1f",money);
	ili9320_PutChars_S3264(60+32*2,64,tab_fee_calc,5,Red,Green);// jine


	ili9320_PutChars_S3264(60+32*2,128,tab_wait_time,5,Red,Green);		  //dengdai

	ili9320_PutChars_S3264(32,128,tab_speedsize,1,Red,Green);		   //dangwei
	
	ili9320_PutChars_S816(8,239-32-16,tab_time,8,Red,Green);
	ili9320_PutChars_S816(8,239-32,tab_date,8,Red,Green);
	}
														  
/*											
	ili9320_Putdata(60+32*2,0,2,Red,Green);	//2
	ili9320_Putdata(60+32*3,0,5,Red,Green); //5
	ili9320_Putdata(60+32*4,0,1,Red,Green);	//1
	ili9320_Putdata(60+32*5,0,10,Red,Green);//.
	ili9320_Putdata(60+32*6-16,0,8,Red,Green);	//8

	ili9320_Put_Km(60+32*7-16,20,Red,Green);  //km

	ili9320_PutCharz(60,20+32+20+10,2,2,Red,Green);	//经俄
	
											
	ili9320_Putdata(60+32*2,64,3,Red,Green);	//2
	ili9320_Putdata(60+32*3,64,6,Red,Green); //5
	ili9320_Putdata(60+32*4,64,8,Red,Green);	//1
	ili9320_Putdata(60+32*5,64,10,Red,Green);//.
	ili9320_Putdata(60+32*6-16,64,9,Red,Green);	//8
	
	ili9320_PutCharz(60+32*7-16,20+32+20+10,1,4,Red,Green);  //元

	ili9320_Putdata(60+32*2,128,10,Red,Green);//.
	ili9320_Putdata(60+32*3,128,9,Red,Green);	//8
*/
